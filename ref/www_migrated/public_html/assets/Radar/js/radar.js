...
const IS_BLOG_PAGE = window.location.pathname.includes('/radar');
...
const CTA_HIBP = `
    <article class="filler-card group border-none bg-gradient-to-br from-[#050A18] via-[#050A18] to-[#8A2BE2] !p-12 overflow-hidden relative flex flex-col justify-center h-full shadow-[0_30px_100px_-20px_rgba(5,10,24,0.8)]">
        ...
        <div class="relative z-10 flex flex-col h-full">
            ...
            <div class="mt-auto">
                <form action="/" method="GET" class="relative group/input">
...
const CTA_LGPD = `
    <article class="filler-card glass-card group border-[#8A2BE2]/50 !p-0 overflow-hidden relative flex flex-col justify-end h-full shadow-[0_30px_80px_-20px_rgba(138,43,226,0.4)]">
        <img src="https://cdn.santis.ddev.site/services/2026/02/santis_control.png" class="absolute inset-0 w-full h-full object-cover opacity-30 grayscale group-hover:grayscale-0 group-hover:opacity-50 transition-all duration-1000">
            ...
            <article class="news-card glass-card glass-card-purple overflow-hidden group !border-white/10 cursor-pointer relative" onclick="openPost(${post.id})">
                ...
                <div class="p-8">
                    <div class="flex justify-between items-center mb-6">
                        <a href="/radar?filter=${post.category}" onclick="event.stopPropagation()" class="px-3 py-1 rounded bg-[#8A2BE2]/10 text-[#8A2BE2] text-[9px] font-black uppercase tracking-widest border border-[#8A2BE2]/20 hover:bg-[#8A2BE2] hover:text-white transition-all">${post.category}</a>
        ...
        if (slotsNeeded === 2) {
            const filler = `
                <div class="filler-card filler-2-cols glass-card group cursor-pointer" onclick="document.getElementById('radar').scrollIntoView({behavior:'smooth'})">
                    <img src="https://cdn.santis.ddev.site/radar/2026/02/hero-santis.png" alt="Scanner">
            ...
            const filler = `
                <div class="filler-card glass-card group cursor-pointer border-[#8A2BE2]/30 bg-[#8A2BE2]/5" onclick="window.location.href='/artigo?id=1'">
